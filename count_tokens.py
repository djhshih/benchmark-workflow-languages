...
- CWL:   Workflow .cwl files = WF, CommandLineTool .cwl files = Tool
- SWL:   .swl files = WF, .sh files = Tool (shell scripts with # in/out/run annotations)
- Nextflow: Single .nf file = WF (no separate tool files)
- WDL:   Single .wdl file = WF (tasks and workflow in same file)
- Python: Single .py file = WF (DSL defines both workflow and tools)
    ...
    for line in lines:
        ...
        if language == 'swl':
            if stripped.startswith('#'):
                if stripped.startswith('#?') or stripped.startswith('# in') or stripped.startswith('# out') or stripped.startswith('# run'):
                    result.append(line)
                continue
        elif language in ['cwl', 'nf', 'wdl', 'ncl']:
            if stripped.startswith('#'):
                continue
        elif language == 'py':
            if stripped.startswith('#'):
...
def main():
    workflows = ['snv', 'cnv']
    languages = ['cwl', 'nf', 'wdl', 'py', 'ncl', 'swl']
    
    results = defaultdict(lambda: {'lines': 0, 'tokens': 0, 'workflow_tokens': 0, 'tool_tokens': 0, 'files': []})
    
    base_dir = Path('.')
    
    ext_map = {'cwl': '.cwl', 'nf': '.nf', 'wdl': '.wdl', 'py': '.py', 'ncl': '.ncl', 'swl': '.swl'}
    dir_map = {'cwl': 'cwl', 'nf': 'nextflow', 'wdl': 'wdl', 'py': 'python', 'ncl': 'nickel', 'swl': 'swl'}
        ...
        for lang in languages:
            ...
            if lang == 'py':
                ...
                for sh_file in lang_dir.glob('*.sh'):
                    ...
                    results[(workflow, lang)]['lines'] += len(content_no_comments.split('\n'))
                    results[(workflow, lang)]['tokens'] += tokens
                    results[(workflow, lang)]['tool_tokens'] += tokens
                ...
                for file_path in lang_dir.rglob(f'*{ext_map[lang]}'):
                    ...
                    is_workflow = 'class: Workflow' in content or 'workflow {' in content or 'workflow snv_calling' in content or 'workflow cnv_calling' in content or 'dfn.Workflow' in content
    ...
    print("-" * 98)
    print(f"{'Workflow':<10} {'Language':<10} {'Files':>6} {'Lines':>7} {'Tokens':>8} {'WF Tok':>8} {'Tool Tok':>9}")
    print("-" * 98)
    ...
    for workflow in workflows:
        for lang in languages:
            ...
            if data['files']:
                ...
                print(f"{workflow:<10} {lang:<10} {file_count:>6} {data['lines']:>7} {data['tokens']:>8} {data['workflow_tokens']:>8} {data['tool_tokens']:>9}")
                total_lines[lang] += data['lines']
                total_tokens[lang] += data['tokens']
                total_wf[lang] += data['workflow_tokens']
                total_tools[lang] += data['tool_tokens']
    ...
    print("-" * 80)
    print(f"{'Language':<10} {'Lines':<8} {'Tokens':<10} {'WF Tokens':<12} {'Tool Tokens':<12} {'Tokens/Line':<12}")
    print("-" * 80)
    for lang in languages:

...
set -e
bwa mem -t ${cpu} -R "@RG\tID:${sample_name}\tLB:1\tPL:ILLUMINA\tSM:${sample_name}" ${reference} ${reads[0]} ${reads[1]} 2> ${sample_name}.bwa.log | samtools sort -@ $((cpu - 1)) -m 2G -o ${sample_name}.sorted.bam -
